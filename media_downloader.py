            ...
            for _type in media_types:
                ...
                if _can_download(_type, file_formats, file_format):
                    ...
                    if download_path:
                        ...
                        if type(_media).__name__ == 'Photo':
                            # print(message.caption)
                            await client.send_photo(target_chat, download_path, caption=message.caption if message.caption else "")
                        if type(_media).__name__ == 'Audio':
                            # print(message.caption)
                            await client.send_audio(target_chat, download_path, caption=message.caption if message.caption else "")
                        if type(_media).__name__ == 'Video':
                            # print(message.caption)
                            await client.send_video(target_chat, download_path, caption=message.caption if message.caption else "")
                        if type(_media).__name__ == 'Document':
                            # print(message.caption)
                            await client.send_document(target_chat, download_path, caption=message.caption if message.caption else "")
...
async def process_messages(
    ...
    for message in messages:
        message_ids.append(await download_media(client, message, media_types, file_formats, target_chat))
...
async def begin_import(config: dict, pagination_limit: int) -> dict:
    ...
    #     print("{} {} {}".format(dialog.chat.title, dialog.chat.id, dialog.chat.type))
    # exit(0)
